...
export default function DashboardPage() {
  ...
  const [activityPanel, setActivityPanel] = useState<'feed' | 'mail'>('feed');
      ...
      <div className="ot-dashboard ot-stagger">
        ...
        <OTCard accent p="sm" style={{ gridArea: 'econ', overflow: 'hidden' }} header={
          ...
          <Stack gap={8}>
            ...
            <TooltipStat label="Gold / turn" value={`+${toLocale(goldPerTurn)}`} valueColor="var(--ot-success)" tooltip={breakdown?.goldPerTurn ? <GoldTooltipContent bd={breakdown.goldPerTurn} /> : 'Loading...'} tooltipWidth={220} />
        ...
        <OTCard p="sm" style={{ gridArea: 'mil', overflow: 'hidden' }} header={
          ...
          <Stack gap={8}>
            ...
            {([
              ...
              return <TooltipStat key={key} label={label} value={toLocale(value)} tooltip={bd ? <StatTooltipContent label={label} bd={bd} detailed={det} /> : 'Loading...'} />;
            })}
          </Stack>
        </OTCard>
        ...
        <OTCard p="sm" style={{ gridArea: 'activity', overflow: 'hidden', display: 'flex', flexDirection: 'column' }} header={
          <Group justify="space-between" align="center">
            ...
            <Group gap={6} className="ot-segmented-pill">
              <button className={activityPanel === 'feed' ? 'active' : ''} onClick={() => setActivityPanel('feed')}>Feed</button>
              <button className={activityPanel === 'mail' ? 'active' : ''} onClick={() => setActivityPanel('mail')}>
                Mail{unreadCount > 0 && <Badge size="xs" color="red" variant="filled" circle ml={6}>{unreadCount}</Badge>}
              </button>
            </Group>
          </Group>
        }>
            ...
            {activityPanel === 'feed' ? (
              <Stack gap={6}>
                <Group gap={6}>
                  ...
                  <Anchor component={Link} href="/world/activity" size="xs" ml="auto">View full feed</Anchor>
                </Group>
              ...
              <Stack gap={8}>
                <Group justify="space-between" align="center">
                  <Text size="xs" fw={700} tt="uppercase" className="ot-text-dim">Latest messages</Text>
