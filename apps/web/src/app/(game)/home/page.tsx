      ...
      <div className="ot-dashboard ot-stagger">
        ...
        <OTCard accent p="sm" style={{ gridArea: 'econ', overflow: 'hidden' }} header={
          ...
          <Stack gap={8}>
            ...
            <TooltipStat label="Total net worth" icon={IconChartArcs} value={toLocale(gold + goldInBank + armoryResaleValue + battleUpgradeResaleValue)} tooltip={<Stack gap={2}><Text size="xs" fw={700}>Net Worth (Resale Value)</Text><Text size="xs">Gold on hand: {toLocale(gold)}</Text><Text size="xs">Gold in bank: {toLocale(goldInBank)}</Text>{armoryResaleValue > 0 && <Text size="xs">Armory (75%): {toLocale(armoryResaleValue)}</Text>}{battleUpgradeResaleValue > 0 && <Text size="xs">Battle upgrades (75%): {toLocale(battleUpgradeResaleValue)}</Text>}<Divider size="xs" color="var(--ot-border-subtle)" /><Text size="xs" fw={600}>Total: {toLocale(gold + goldInBank + armoryResaleValue + battleUpgradeResaleValue)}</Text></Stack>} tooltipWidth={240} />
          </Stack>
        </OTCard>
